...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            # SetModbusDoubleCheck指令格式
            #
            # 寫入指定的TCP Modbus
            #
            #["SetModbusDoubleCheck",[IP位址(字串), port(Int)],[Slave ID(Int), 寫入的儲存欄位(字串)],[開始的位址(Int),寫入的數量(Int),寫入的數值(Int/List)], Timeout(Float)]
            #
            # 
            # 寫入的儲存欄位: "HOLD_REG" => Holding Register
            #                 "COIL" => Coil
            #
            # 寫入的數值: 900 => Int
            #             [10, 30, 600, 55] => List
            ...
            "task_arr":[ 
                            ["SetModbusDoubleCheck",["172.16.49.250",502],[1,"HOLD_REG"],[0,1,900], -1.0]
                       ] 
