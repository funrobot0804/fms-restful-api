...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            # PatternDockTo_LeaveCenter指令格式
            #
            # 命令機器離開停靠的中央辨識板
            #
            # ["PatternDockTo_LeaveCenter",[distance(float, 單位:m), is avoid(int)]] 
            #                  
            # distance: 遠離Pattern的距離，單位:m(公尺)
            #           小於 0.0: 使用預設值
            #           (目前尚無作用，使用預設值)
            #
            #                        
            ...
            "task_arr":[ ["PatternDockTo_LeaveCenter",[-1.0, 1]]
                       ] 
