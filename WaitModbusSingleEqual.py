...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            # 寫入指定的TCP Modbus
            #
            #["SetModbus",[IP位址(字串), port(Int)],[Slave ID(Int), 寫入的儲存欄位(字串)],[開始的位址(Int),對比的數值(Int)],Timeout(Float)]
            #
            # 
            # 寫入的儲存欄位: "HOLD_REG" => Holding Register
            #                 "INPUT_REG" => Input Register
            #                 "DISC_INPUT" => discrete input
            #                 "COIL" => Coil
            #
            # 寫入的數值: 900 => Int
            #             
            ...
            "task_arr":[ 
                            ["WaitModbusSingleEqual",["172.16.49.250",502],[1,"COIL"],[0,1], -1.0]
                       ] 
