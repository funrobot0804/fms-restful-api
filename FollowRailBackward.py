...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            # FollowRailBackward指令格式 
            #
            # 火車模式，沿著指定軌跡走，'不會'繞過障礙物，倒著走
            #
            #["FollowRailBack", [rail_name, rail_startpos_index, rail_endpos_index, floor] ]
            #rail_name: 字串，欲使用rail的名稱
            #rail_startpos_index: int，使用rail的起始位置index (0為rail的起點，終點為rail的長度-1)    
            #rail_endpos_index: int，使用rail的終點或欲到達位置index (0為rail的起點，終點為rail的長度-1)
            #floor: 字串，rail的所在樓層
            #
            "task_arr":[ ["FollowRailBackward", ["rail_No.345", 0, 700, "1F"] ] \
                       ] 
