...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            "robot_mode":[0,0,0],                 # 格式為list，0 => off
                                                  #             1 => on
                                                  # [上UV燈、下UV燈、吸塵器]
            "robot_name":"msi_48",             # 格式為str，指定機器的名字，如果有指定就只會挑選特定名稱的機器
                                                  
            # ExtOutputSetMuti指令格式
            #
            # 輸出外部訊號
            #
            # ["ExtOutputSetMuti", input list of index (List of Int，指定欲等待按下按鈕的編號), 
            #                      input list of index signal (List of Int, 1:High, 0:Low)
            #                 
            # ]
            "task_arr":[ ["ExtOutputSetMuti", [0,1,2,3], [0,1,1,0]],
                         ["TimeWait", 20  ]            
                       ] 
