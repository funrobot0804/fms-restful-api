...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            "robot_mode":[0,0,0],                 # 格式為list，0 => off
                                                  #             1 => on
                                                  # [上UV燈、下UV燈、吸塵器]
            "robot_name":"msi_44",             # 格式為str，指定機器的名字，如果有指定就只會挑選特定名稱的機器
                                                  
            # ExtInputWaitMuti指令格式
            #
            # 等待外部訊號輸入
            #
            # ["ExtInputWaitMuti", List of input index(List of Int，指定輸入Input pin腳的編號), 
            #                 List of input signal(List of Int, 1:High, 0:Low), 
            #                 timeout(float，單位為秒)
            # ]
            "task_arr":[ ["ExtInputWaitMuti", [1,2], [0,0] ,120],
                         ["TimeWait", 20  ]             
                       ] 
