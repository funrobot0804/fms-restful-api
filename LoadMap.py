...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            "robot_name": "msi_delta",            # 格式為str，指定機器的名字，如果有指定就只會挑選特定名稱的機器
                                                  
            # Map相關指令格式              
            # ["LoadMap", [map_folder_name] ]  map_folder_name => str   (載入地圖)
            # ["SaveMap", [map_folder_name] ]  map_folder_name => str   (儲存地圖) 
            # ["Relocate", [[x,y,angle]] ]     x=> float, unit:m        (重新定位)
            #                                  y=> float, unit:m
            #                                  angle=> float, unit: rad
            "task_arr":[ ["LoadMap", ["msi3F_1a"] ],      \
                         ["LoadMap", [""] ],             \
                         ["SaveMap", ["msi3F_1a"] ],      \
                         ["SaveMap", [""] ],             \
                         ["Relocate", [0.,0.,3.14] ],  \
