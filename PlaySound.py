...
payload = { "start_time":float( time.time() ),    # 格式為float，西元1970年後所經過的浮點秒數(UTC)         
            ...
            "robot_name": "msi_alpha",            # 格式為str，指定機器的名字，如果有指定就只會挑選特定名稱的機器
                                                  
            # PlaySound指令格式              
            # ["PlaySound", [music_filename,play_times, wait_next_n_behavior_to_stop] ]    
            # 
            # music_filename: 字串，欲撥放音樂的檔案名稱和所在位置 Ex. "/data/wav/bullfrog.mp3"
            # play_times: int，重複播放的次數    
            # wait_next_n_behavior_to_stop: int，在下n個任務停止時停止
            #
            #
            #
            # ["PlaySound", ["music_filename", "music_filename", "music_filename"...] ,play_times, wait_next_n_behavior_to_stop] ]    
            # 
            # ["music_filename1", "music_filename2", "music_filename3"...]: 字串組成的list，可以指定一連串的欲撥放音樂的檔案名稱和所在位置
            # play_times: int，重複播放的次數    
            # wait_next_n_behavior_to_stop: int，在下n個任務停止時停止  
            "task_arr":[ ["PlaySound", ["/data/wav/bullfrog.mp3",1, 0] ],      \
                         ["PlaySound", [ ["/data/wav/Hello.mp3", "/data/wav/Welcome.mp3", "/data/wav/GoodBye.mp3"] ,2, 2] ]      \
                       ] 
